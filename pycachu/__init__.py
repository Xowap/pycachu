...
class Pycachu(object):
    ...
    def get(self, key):
        ...
        try:
            f = open(self.make_path(h), 'rb')
            fcntl.lockf(f, fcntl.LOCK_SH)

            c = self.db.cursor()
            c.execute(
                '''UPDATE "file" SET last_access = ? WHERE hash = ?''',
                (int(time.time()), h)
            )
            self.db.commit()
    ...
    def put(self, key, content, commit=True):
        h = self.hash(key)
        p = self.make_path(h)
        ...
        c = self.db.cursor()
        c.execute('''
          INSERT INTO "file" ("hash", "key", "size", "last_access")
          VALUES (?, ?, ?, ?)
        ''', (h, key, size, int(time.time())))
    ...
    def make_path(self, hash):
        return os.path.join(self.root, hash[0:2], hash[2:4], hash)
